...
multiplicative rules is the log-spectral residual (or Itakura--Saito
divergence), not the L\textsuperscript{2} hologram-plane error; this resolves
the apparent paradox that the hologram-plane correction vector can grow
transiently while the image-plane error decreases. On a benchmark of ten
natural grayscale images, Overdrive at $\beta=0.7$ yields a mean PSNR of
$32.19$~dB after twenty iterations, an improvement of $7.43$~dB over plain GS
and $5.41$~dB over the Fienup amplitude variant. A reference Python
...
\cite{Fienup1978,Fienup1980,Prongue1992,Bengtsson1994,Liu2002} solve the
...
\cite{Zheng2021,Sun2022,Zhu2025} sidestep iteration altogether at the cost of
training data and inference hardware. For real-time video projection on
...
intensities. Decomposing $v_k=v^*+\delta v_k$ around the limit cycle and using
...
the transient part of the log-spectral metric, which is the natural metric
for a multiplicative iteration. This is consistent with classical results in
spectral estimation, where Itakura--Saito has long been the preferred
dissimilarity measure for multiplicative spectral updates.
...
\paragraph{Relation to other multiplicative IFTA rules, continued.} Our
synthesis-floor formula \eqref{eq:ssresid} also illuminates why the Bengtsson
rule degrades on grayscale targets: with a closed-loop pole at $z=1-c\beta$
and no $\F^{2-\beta}$ scaling to balance dimensions, the noise term $\eta_k$
is amplified rather than filtered. The Overdrive's exponent $2-\beta$ is
exactly the gain that keeps the closed loop stable while preserving the
fixed-point property in the (hypothetical) zero-residual limit.
...
for this multiplicative rule; the log-spectral residual is. Numerical
experiments on ten natural grayscale images confirm the prediction of an
interior optimum in $\beta$ and show a $7.4$~dB PSNR advantage of Overdrive
