...
$32.40$~dB after twenty iterations, an improvement of $7.46$~dB over plain GS
and $5.43$~dB over the Fienup amplitude variant. A reference Python
...
$\beta_{\mathrm{BE}}=0.35$) on 29 natural grayscale images of resolution
...
convention); ``unscaled'' does not. Overdrive dominates at all three
$\beta$ values; the optimum on this set lies near $\beta=0.7$.}
\label{tab:results}
\small
...
beats GS by $7.46$~dB, Fienup--Amplitude by $5.43$~dB, and Bengtsson by
$9.15$~dB, on average. The non-monotone behaviour in $\beta$
...
simplicity (two square roots) \cite{Bernau2012}; our results suggest the
optimal value is consistently larger. The unscaled and scaled PSNR differ by
only $0.3$--$0.6$~dB across rules, with unscaled values being uniformly
higher: the scaled metric of \cite{Bernau2012} therefore neither inflates
nor reorders the comparison.
